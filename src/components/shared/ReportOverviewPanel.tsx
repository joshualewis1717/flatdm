...
function ReportOverviewItem( {report} : ScriptProps){//{id, reason, createdAt, reporter, targetUser} : Report){
...
export default function ReportOverviewPanel( {reports}:ScriptProps ){
            ...
            <section className="flex flex-col gap-2 py-[3%]">
                {
                    ...
                    reports.map( (report : Report, key : string) => (
                        <div key={report['id']}>
                            <ReportOverviewItem  report={report}/>
                        </div>
