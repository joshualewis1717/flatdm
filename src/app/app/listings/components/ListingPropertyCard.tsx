...
function PropertyCardAvailabilityBadge({ isAvailableNow }: { isAvailableNow: boolean }) {
  return (
...
function PropertyCardAvailabilityInfo({
  availableFrom,
  updatedAt,
  maxOccupants,
  area,
  ...
  return (
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="text-right self-start">
        <div className="text-2xl font-semibold text-[#c9fb00]">{formatCurrency(listing.rent)}</div>
...
function PropertyCardRoomStayInfo({ listing }: { listing: any }) {
  return (
...
function PropertyCardDesc({ description }: { description?: string }) {
  if (!description) {
    return null;
  }

  return <p className="mt-4 line-clamp-2 text-sm leading-6 text-white/60">{description}</p>;
}

function PropertyCardNearbyAmenities({ nearbyAmenities }: { nearbyAmenities: any[] }) {
  return (
